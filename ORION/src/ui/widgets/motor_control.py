"""Motor control panel and quick actions."""
...
class MotorControlWidget(QtWidgets.QGroupBox):
    ...
    step_requested = QtCore.pyqtSignal(float)
    ...
    def _build_row_3(self) -> None:
        ...
        self.btn_reset_safe.setStyleSheet(
            """
            QPushButton { padding: 0px; color: %s; font-weight: bold; border: none; background: transparent; }
    ...
    def set_safe_z_display(self, val: float) -> None:
        self.lbl_safe_val.setText(f"{val:.2f}mm")

    def emit_move_requested(self) -> None:
